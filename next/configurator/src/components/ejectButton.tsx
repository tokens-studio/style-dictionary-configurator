...
import { PlugIcon } from '@iconicicons/react';
...
function getDepsCJSString(dependencies, hasThemes: boolean) {
    ...
    const depsCJSString = dependencies
        .map((dep) => {
            ...
            if (hasNamed && hasDefault) {
                str += `\nconst { ${namedImportsStr(dep.specifiers)} } = ${dep.specifiers.find((spec) => spec.default).name
                    };`;
            }
...
export const EjectButton = () => {
    const onClick = useCallback(async function ejectHandler() {
    ...
    return <Button variant="secondary" id="eject-btn" title="Eject by downloading your project so that you can run it locally." icon={<PlugIcon />}> Eject
        <input
